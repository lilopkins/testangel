...
use testangel::types::{Action, InstructionConfiguration, InstructionParameterSource};
...
impl Into<InstructionParameterSource> for PossibleOutput {
    fn into(self) -> InstructionParameterSource {
        InstructionParameterSource::FromOutput(self.step, self.id, self.friendly_name)
    }
}
            ...
            if ui.button(format!("Step {}", index + 1)).clicked() {
                ...
                for step in instructions.iter_mut() {
                    for (_param_id, param_source) in &mut step.parameter_sources {
                        if let InstructionParameterSource::FromOutput(step, _id, _name) =
                            param_source
                        {
                            if *step == index {
                                *param_source = InstructionParameterSource::Literal;
                            } else if *step > index {
                                *step -= 1;
                            }
                        }
                ...
                for (_output_name, _output_kind, output_source) in outputs.iter_mut() {
                    match output_source {
                        InstructionParameterSource::FromOutput(step, _id, _name) => {
                            if *step == index {
                                *output_source = InstructionParameterSource::Literal;
                            } else if *step > index {
                                *step -= 1;
                            }
                        }
...
impl UiComponent for ActionState {
    fn menu_bar(&mut self, ui: &mut egui::Ui) -> Option<super::State> {
        ...
        ui.menu_button("Action", |ui| {
            if ui.button("New").clicked() {
                ...
                next_state = Some(super::State::ActionEditor);
            }
    ...
    fn always_ui(&mut self, ctx: &egui::Context) -> Option<super::State> {
        ...
        // handle error modal
        let error_modal = super::modals::error_modal(ctx, "action_editor_error_modal", &self.error);
                    ...
                    if ui.button(format!("Delete parameter {}", i + 1)).clicked() {
                        ...
                        for inst in &mut target.instructions {
                            for (_id, src) in &mut inst.parameter_sources {
                                match src {
                                    InstructionParameterSource::FromParameter(param, name) => {
                                        if *param == i {
                                            *src = InstructionParameterSource::Literal;
                                        } else if *param > i {
                                            *src = InstructionParameterSource::FromParameter(
                                                *param - 1,
                                                name.clone(),
                                            );
                                        }
                        ...
                        .show_ui(ui, |ui| {
                            for po in &self.possible_outputs {
                                if po.kind == *output_kind {
                                    let ps: InstructionParameterSource = po.clone().into();
                ...
                ui.group(|ui| {
                    ...
                    for param_id in instruction.parameter_order() {
                        ...
                        ui.horizontal_wrapped(|ui| {
                            ...
                            egui::ComboBox::from_id_source(format!("{index}_param_{param_id}"))
                                ...
                                .show_ui(ui, |ui| {
                                    ui.selectable_value(
                                        param_source,
                                        InstructionParameterSource::Literal,
                                        InstructionParameterSource::Literal.text_repr(),
                                    );

                                    let mut p_id = 0;
                                    for (p_name, p_kind) in &target.parameters {
                                        if *p_kind == *param_kind {
                                            let ps = InstructionParameterSource::FromParameter(
                                                p_id,
                                                p_name.clone(),
                                            );
                                            ui.selectable_value(
                            ...
                            if let InstructionParameterSource::Literal = param_source {
                                // Literal
